...
function getGdImageHandler($filePath)
{
...
function updateImage($sourceImageFilePath, $baseCacheDir, $width, $height)
{
	...
	$fileModify = $baseCacheDir . $relative;
	$fileModifyDir = dirname($fileModify);
	$fileModifyDate = file_exists($fileModify) ? filectime($fileModify) : 0;

	if (!is_dir($fileModifyDir))
	{
		if(!mkdir($fileModifyDir, 0777, true))
		{
			die('Could not create directory for saving the cropped image: ' . $fileModifyDir);
		}
	}
...
function cropImage($filePath, $width, $height, $outputFile)
{
	try
	{
		...
		else
		{
			...
			$proportions = $originalWidth / $originalHeight;
			$proportionsCrop = $width / $height;

			if ($proportions > $proportionsCrop)
			{
				$resizeWidth = ceil($originalWidth / ($originalHeight/$height));
				$resizeHeight = $height;
				$offsetX = abs(($resizeWidth - $width) / 2);
				$offsetY = 0;
			}
			else
			{
				...
				$offsetX = 0;
				$offsetY = abs(($resizeHeight - $height) / 2);
			}
...
function getResizeDimensions($filePath, $width, $height)
{
	...
	{
		/*
		 * we could just copy the original image to the cache folder
		 * but to keep the file sizes small (compression) convert any way
		 */
		$width = $originalWidth;
...
function checkPath($path)
{
	...
	$baseDirLength = strlen(IMAGE_BASE_DIR);
	$appDirLength = strlen(APP_DIR);

	if (substr($realPath, 0, $baseDirLength) != IMAGE_BASE_DIR && substr($realPath, 0, $appDirLength) != APP_DIR)
	{
...
{
	return htmlentities($string, null);
}

function htmlTagOpen($tag, $attributes = array())
{
	$html = '<' . $tag;
	foreach ($attributes as $key => $value)
	{
		$html .= ' ' . htmlEscape($key) . '="' . htmlEscape($value) . '"';
	}
	$html .= '>';
	return $html;
}

function htmlTagContent($tag, $content, $attributes = array())
{
	$html	= htmlTagOpen($tag, $attributes);
	$html .= htmlEscape($content);
	$html .= htmlTagClose($tag);
	return $html;
}
...
function htmlTagSingle($tag, $attributes = array())
{
	$html = htmlTagOpen($tag, $attributes);
